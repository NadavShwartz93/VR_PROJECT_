...
public class Player
{
    ...
    public Player(string hand_in_therapy, string id, string first_name, string last_name, float height, float arm_length, double learning_rate, double discount_factor, double random_explore, int bubble_time_out, int treatment_time, string reward_table, string last_appearance, string qtable, int N, int iterations_number, Vector3 lastBubblePos, float prevSessionVelocityAverage, float prevSessionJerkAverage)
    {
        ...
        Dataset.Get_instance().New_PatientDetailes(id, hand_in_therapy, height,
            arm_length, "yes", treatment_time);

        //Initialize the simulateUseVector from Globals class.
        Globals.simulateUseVector[0] = 0;
        Globals.simulateUseVector[1] = hand_in_therapy == "left" ? 0 : 1;
        Globals.simulateUseVector[2] = (double)height;
        Globals.simulateUseVector[3] = (double)arm_length;
        Globals.simulateUseVector[4] = 1;
        Globals.simulateUseVector[5] = (double)treatment_time;
